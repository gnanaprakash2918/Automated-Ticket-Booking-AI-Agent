...
class OllamaParser(AbstractBusParser):
    ...
    async def _parse_bus_with_llm(
        ...
        for attempt in retry_config:
            with attempt:
                ...
                log.debug(f"Bus {bus_index}: Sending to Ollama (Attempt {attempt.retry_state.attempt_number})")
                ...
                try:
                    ...
                    log.info(f"Bus {bus_index} SUCCESS: {bus_service.operator} - {bus_service.trip_code}")
                    return bus_service

                except Exception as e:
                    log.warning(f"Bus {bus_index}: Ollama parsing failed on attempt {attempt.retry_state.attempt_number}: {e}")
                    raise
    ...
    async def _wrapper_parse_chunk(
        ...
        async with semaphore:
            log.debug(f"OllamaParser: [SEMAPHORE ACQUIRED] Bus {idx}. Active slots: {OLLAMA_CONCURRENCY_LIMIT - semaphore._value}")
            try:
    ...
    async def parse(
        ...
        """
        log.info(f"Using OllamaParser (Interface Loader strategy). Concurrency Limit: {OLLAMA_CONCURRENCY_LIMIT}")
        ...
        log.info(f"OllamaParser: Completed. {len(bus_services)}/{len(bus_divs)} parsed successfully.")
