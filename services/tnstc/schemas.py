...
class TNSTCSearchRequest(BaseModel):
...
class TNSTCResponseMetadata(BaseModel):
...
class TNSTCBusSearchResponse(BaseModel):
    """
    Final output model for the bus search, including metadata like place names and internal codes.
    """
    from_place: TNSTCPlaceInfo = Field(..., description="Details of the confirmed starting place.")
    to_place: TNSTCPlaceInfo = Field(..., description="Details of the confirmed destination place.")
    services: List[TNSTCBusService] = Field(..., description="List of available bus services.")
    metadata: TNSTCResponseMetadata = Field(..., description="Metadata about the search operation.")
...
class TNSTCBusServiceWithReasoning(TNSTCBusService):
    """
    Temporary schema used by the LLM to include its reasoning.
    Inherits all fields and validators from TNSTCBusService.
    """
