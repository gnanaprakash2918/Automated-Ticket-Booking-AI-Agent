...
async def get_place_info(
    client: httpx.AsyncClient, place_name: str, is_from_place: bool
) -> PlaceInfo:
    """
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"External API network error during place lookup: {e}",
        )
    ...
    log.info(
        f"Place lookup SUCCESS for '{place_name}': ID={parts[0]}, Code={parts[1]}, Name='{parts[2]}'"
    )
...
async def parse_bus_results(
    client: httpx.AsyncClient, html_content: str, limit: Optional[int] = None
) -> List[BusService]:
    """
    ...
    log.info(
        f"Calling bus parser with strategy: {parser.__class__.__name__}. Limit: {limit if limit is not None else 'None'}."
    )
    ...
    try:
        ...
        log.error(
            f"Unhandled error during parsing strategy '{parser.__class__.__name__}': {e}",
            exc_info=True,
        )
        return []
...
def filter_bus_services(
    bus_list: List[BusService], request: SearchRequest
) -> List[BusService]:
    ...
    max_price = (
        request.max_price_in_rs if request.max_price_in_rs is not None else float("inf")
    )
    ...
    log.info(
        f"Applying filters: Price ({min_price}-{max_price}), Time ({min_dep_str}-{max_dep_str}), Types: {allowed_types_lower if allowed_types_lower else 'All'}"
    )
    ...
    for service in bus_list:
        try:
            ...
            if not re.fullmatch(r"([01]\d|2[0-3]):[0-5]\d", service.departure_time):
                log.warning(
                    f"Skipping service with invalid departure time: {service.departure_time}"
                )
                continue
            ...
            if price_ok and time_ok and type_ok:
                ...
                log.debug(
                    f"Service {service.trip_code} filtered out: Price OK={price_ok}, Time OK={time_ok}, Type OK={type_ok}"
                )
