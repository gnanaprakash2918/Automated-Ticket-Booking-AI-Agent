...
def get_parser() -> BusParser:
    ...
    if _parser_instance is not None:
        ...
        if (
            (
                isinstance(_parser_instance, GeminiParser)
                and current_strategy == "gemini"
            )
            or (
                isinstance(_parser_instance, BeautifulSoupParser)
                and current_strategy == "beautifulsoup"
            )
            or (
                isinstance(_parser_instance, OllamaParser)
                and current_strategy == "ollama"
            )
        ):
    ...
    if PARSER_STRATEGY == "gemini":
        ...
        try:
            ...
            log.error(
                f"Failed to initialize GeminiParser: {e}. Defaulting to 'beautifulsoup'."
            )
        ...
        try:
            ...
            log.error(
                f"Failed to initialize OllamaParser: {e}. Defaulting to 'beautifulsoup'."
            )
            _parser_instance = BeautifulSoupParser()
        ...
        log.error(
            f"Invalid PARSER_STRATEGY: '{PARSER_STRATEGY}'. Defaulting to 'beautifulsoup'."
        )
        _parser_instance = BeautifulSoupParser()
