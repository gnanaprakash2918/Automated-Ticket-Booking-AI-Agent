...
async def check_health():
    ...
    return {"status": "ok", "message": "TNSTC API Wrapper is running."}
...
@app.post(
    "/search_buses", response_model=BusSearchResponse, status_code=status.HTTP_200_OK
)
async def search_buses(
    ...
    log.info(
        f"Received search request: {request.from_place_name} -> {request.to_place_name} on {request.onward_date}"
    )

    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            log.info("Starting concurrent place lookups.")
            from_place_task = get_place_info(
                client, request.from_place_name, is_from_place=True
            )
            to_place_task = get_place_info(
                client, request.to_place_name, is_from_place=False
            )
        ...
        log.info(
            f"Executing external search API call. Payload data keys: {list(payload.keys())[:5]}..."
        )
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"External API network request failed: {e}",
            )
