...
def minify_html(
    ...
    for t in unwrap_tags + ["span", "small"]:
        for tag in list(soup.find_all(t)):
            if tag.attrs and any(
                k in ("id", "class")
                or k.startswith(("data-", "aria-"))
                or k in preserve_attrs
    ...
    def keep_attr(a: str) -> bool:
        return (
            a in ("id", "class")
            or a.startswith(("data-", "aria-"))
            or a in preserve_attrs
        )
    ...
    def convert_simple_tables(soup_obj):
        for table in list(soup_obj.find_all("table")):
            ...
            for k, v in kvs:
                kv = frag.new_tag("div")
                kv["class"] = "kv"
                ks = frag.new_tag("span")
                ks["class"] = "k"
                ks.string = k
                vs = frag.new_tag("span")
                vs["class"] = "v"
                vs.string = v
                kv.append(ks)
                kv.append(vs)
    ...
    for b in list(soup.find_all("b", class_="boxheader")):
        h = soup.new_tag("h2")
        h["class"] = "boxheader"
        h.string = b.get_text(strip=True)
        b.replace_with(h)
    ...
    for tag in list(soup.find_all(True)):
        if (
            ...
            and not any(isinstance(c, Tag) for c in tag.contents)
            and tag.name in removable
        ):
            tag.decompose()
...
def extract_examples_from_pydantic_schema(
    model: Type[BaseModel], visited: set = None
) -> str:
    if visited is None:
        visited = set()
    if model in visited:
